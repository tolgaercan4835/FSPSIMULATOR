...
const EvaluationModal: React.FC<EvaluationModalProps> = ({ isOpen, onClose, evaluationResult }) => {
    ...
    const scoreData = useMemo(() => {
        if (!evaluationResult) return { score: null, color: 'text-gray-200' };
    ...
    const formattedResult = evaluationResult.replace(/\*\*Puan:\*\*\s*\d+\/100/, (match) => {
        ...
        >
            ...
            >
                <header className="flex justify-between items-center p-4 border-b border-gray-700 sticky top-0 bg-gray-800">
                    <h2 id="evaluation-title" className="text-xl font-bold text-gray-100">Sınav Sonuç Karnesi</h2>
                    <button 
                        onClick={onClose} 
